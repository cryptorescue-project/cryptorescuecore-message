...
var ECDSA = cryptorescuecore.crypto.ECDSA;
var Signature = cryptorescuecore.crypto.Signature;
var sha256sha256 = cryptorescuecore.crypto.Hash.sha256sha256;
...
Message.MAGIC_BYTES = new Buffer('CryptoRescue Signed Message:\n');
...
/**
 * Will sign a message with a given cryptorescue private key.
 *
...
/**
 * Will return a boolean of the signature is valid for a given cryptorescue address.
 * If it isn't the specific reason is accessible via the "error" member.
 *
 * @param {Address|String} cryptorescueAddress - A cryptorescue address
 * @param {String} signatureString - A base64 encoded compact signature
 * @returns {Boolean}
 */
Message.prototype.verify = function verify(cryptorescueAddress, signatureString) {
  $.checkArgument(cryptorescueAddress);
  $.checkArgument(signatureString && _.isString(signatureString));

  if (_.isString(cryptorescueAddress)) {
    cryptorescueAddress = Address.fromString(cryptorescueAddress);
  }
  ...
  var signatureAddress = Address.fromPublicKey(publicKey, cryptorescueAddress.network);

  // check that the recovered address and specified address match
  if (cryptorescueAddress.toString() !== signatureAddress.toString()) {
